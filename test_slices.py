...
def plot():
    ...
    for batch_size in sorted(map(int, list(data.keys()))):
        ...
        num_splits_ll = sorted(map(int, list(dd.keys())))
        time_per_step_ll = list(dd[str(ns)]['time_per_step'] for ns in num_splits_ll)
        plt.plot(num_splits_ll, time_per_step_ll, label='Batch size=%d'%batch_size)
